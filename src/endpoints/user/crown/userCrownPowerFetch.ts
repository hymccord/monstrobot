...
export class UserCrownPowerFetch extends OpenAPIRoute {
    schema = {
        tags: ["User"],
        summary: "Get a user's power crown summary",
        request: {
            params: z.object({
                userSlug: z.number().describe("User slug"),
                powerTypeSlug: powerCrownTypesSchema.describe("Crown type"),
            }),
            headers: z.object({
                hgToken: z
                    .string()
    ...
    async handle(c: Context) {
        ...
        const { userSlug, powerTypeSlug } = data.params;
